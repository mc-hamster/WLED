...
namespace {
enum class ConfigWriteOutcome { Deferred, Succeeded, Failed };
ConfigWriteOutcome serializeConfigSecChecked();
std::atomic<uint32_t> completedConfigWrites{0};
std::atomic<bool> lastConfigWriteSucceeded{false};

// Stage a complete JSON document before replacing its previous version. Counts
// and reopened file size catch failed/short writes before acknowledging a save.
bool writeConfigDocument(const String& path, JsonObject root) {
  if (pDoc->overflowed()) return false;
  const String staged = path + F(".tmp");
  File output = WLED_FS.open(staged, "w");
  if (!output) return false;
  const size_t expected = measureJson(root);
  const size_t written = serializeJson(root, output);
  output.flush();
  bool complete = written == expected && output.size() == expected;
  output.close();
  if (complete) {
    File verify = WLED_FS.open(staged, "r");
    complete = verify && verify.size() == expected;
    verify.close();
  }
  if (complete) complete = WLED_FS.rename(staged, path);
  if (!complete) WLED_FS.remove(staged);
  return complete;
}

void finishConfigWrite(bool succeeded) {
  lastConfigWriteSucceeded.store(succeeded, std::memory_order_relaxed);
  completedConfigWrites.fetch_add(1, std::memory_order_release);
  configNeedsWrite = false;
  if (!succeeded) errorFlag = ERR_FS_GENERAL;
}
}

uint32_t configWriteGeneration() { return completedConfigWrites.load(std::memory_order_acquire); }
bool configWriteSucceeded() { return lastConfigWriteSucceeded.load(std::memory_order_relaxed); }
// AI: end
...
void serializeConfigToFS() {
  const ConfigWriteOutcome secureOutcome = serializeConfigSecChecked();
  if (secureOutcome == ConfigWriteOutcome::Deferred) return;
  if (secureOutcome == ConfigWriteOutcome::Failed) {
    finishConfigWrite(false);
    return;
  }
  ...
  const bool succeeded = writeConfigDocument(String(FPSTR(s_cfg_json)), root);
  releaseJSONBufferLock();
  finishConfigWrite(succeeded);
}
...
ConfigWriteOutcome serializeConfigSecChecked() {
  ...
  if (!requestJSONBufferLock(JSON_LOCK_CFG_SEC_SER)) return ConfigWriteOutcome::Deferred;
  ...
  const bool succeeded = writeConfigDocument(String(FPSTR(s_wsec_json)), root);
  if (!succeeded) errorFlag = ERR_FS_GENERAL;
  releaseJSONBufferLock();
  return succeeded ? ConfigWriteOutcome::Succeeded : ConfigWriteOutcome::Failed;
}
}

void serializeConfigSec() {
  serializeConfigSecChecked();
}
