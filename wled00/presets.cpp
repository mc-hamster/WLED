...
namespace {
std::atomic<uint32_t> completedPresetWrites{0};
std::atomic<bool> lastPresetWriteSucceeded{false};

// Close the shared filesystem writer before publishing completion. Its next
// reader must observe committed bytes, not a still-buffered accepted command.
void finishPresetWrite(bool succeeded) {
  if (doCloseFile) closeFile();
  lastPresetWriteSucceeded.store(succeeded, std::memory_order_relaxed);
  completedPresetWrites.fetch_add(1, std::memory_order_release);
  if (!succeeded && errorFlag == ERR_NONE) errorFlag = ERR_FS_GENERAL;
}
}

uint32_t presetWriteGeneration() { return completedPresetWrites.load(std::memory_order_acquire); }
bool presetWriteSucceeded() { return lastPresetWriteSucceeded.load(std::memory_order_relaxed); }
// AI: end
...
*/
  ...
  if (!persist) {
    ...
    if (tmpRAMbuffer!=nullptr) {
      succeeded = !pDoc->overflowed() && serializeJson(*pDoc, tmpRAMbuffer, len) == len - 1;
      if (!succeeded) { p_free(tmpRAMbuffer); tmpRAMbuffer = nullptr; }
    } else {
      succeeded = writeObjectToFileUsingId(getPresetsFileName(persist), presetToSave, pDoc);
    }
  } else
  #endif
  succeeded = writeObjectToFileUsingId(getPresetsFileName(persist), presetToSave, pDoc);

  finishPresetWrite(succeeded);
  if (persist && succeeded) {
    presetsModifiedTime = toki.second(); //unix time
    cacheInvalidate++; // two saves in one second still invalidate HTTP's PSRAM cache
  }
  ...
  if (sObj.size()==0 || sObj["o"].isNull()) { // no "o" means not a playlist or custom API call, saving of state is async (not immediately)
    ...
    if (sObj[F("playlist")].isNull()) {
      ...
      if (index <= 250) { // cannot save API calls to temporary preset (255)
        ...
        initPresetsFile(); // just in case if someone deleted presets.json using /edit
        const bool succeeded = writeObjectToFileUsingId(getPresetsFileName(), index, pDoc);
        finishPresetWrite(succeeded);
        if (succeeded) { presetsModifiedTime = toki.second(); cacheInvalidate++; }
        updateFSInfo();
      } else finishPresetWrite(false);
      p_free(saveName);
      p_free(quickLoad);
      saveName = nullptr;
      quickLoad = nullptr;
...
void deletePreset(byte index) {
  ...
  const bool succeeded = writeObjectToFileUsingId(getPresetsFileName(), index, &empty);
  finishPresetWrite(succeeded);
  if (succeeded) { presetsModifiedTime = toki.second(); cacheInvalidate++; }
  updateFSInfo();
}
