...
namespace {
// Keep HTTP parsing and response ownership at the transport boundary.
class HttpSettingsForm final : public WLEDSettingsForm {
  AsyncWebServerRequest& _request;
public:
  explicit HttpSettingsForm(AsyncWebServerRequest& request) : _request(request) {}
  String arg(const String& name) const override { return _request.arg(name); }
  String arg(size_t index) const override { return _request.arg(index); }
  String argName(size_t index) const override { return _request.argName(index); }
  size_t args() const override { return _request.args(); }
  bool hasArg(const String& name) const override { return _request.hasArg(name.c_str()); }
};
}

// Returns true after handling/defering a response so serveSettings cannot send
// a second success page after a reset, failed save, or busy usermod update.
bool handleSettingsSet(AsyncWebServerRequest *request, byte subPage) {
  ...
  HttpSettingsForm form(*request);
  switch (applySettingsForm(form, subPage)) {
    case SettingsFormResult::Busy:
      request->deferResponse();
      return true;
    case SettingsFormResult::Reset:
      serveMessage(request, 200, F("All Settings erased."), F("Connect to WLED-AP to setup again"), 255);
      return true;
    case SettingsFormResult::StorageFailure:
      serveMessage(request, 500, F("Settings could not be erased."), F("Check the filesystem and try again."), 254);
      return true;
    case SettingsFormResult::Invalid:
      serveMessage(request, 400, F("Invalid settings page."), F("Reload settings and try again."), 254);
      return true;
    case SettingsFormResult::Applied:
      return false;
  }
  return false;
}
...
{
  WLEDSettingsForm* request = &form;
  //0: menu 1: wifi 2: leds 3: ui 4: sync 5: time 6: sec 7: DMX 8: usermods 9: N/A 10: 2D
  if (subPage < 1 || subPage > 10 || subPage == 9) return SettingsFormResult::Invalid;
  #ifndef WLED_ENABLE_DMX
  if (subPage == SUBPAGE_DMX) return SettingsFormResult::Invalid;
  #endif
  #ifdef WLED_DISABLE_2D
  if (subPage == SUBPAGE_2D) return SettingsFormResult::Invalid;
  #endif
  ...
  if (subPage == SUBPAGE_SEC)
  {
    if (request->hasArg(F("RS"))) //complete factory reset
    {
      ...
      if (!WLED_FS.format()) {
        ...
        return SettingsFormResult::StorageFailure;
      }
      configNeedsWrite = false;
      doReboot = true; // transport adapters must finish their response before rebooting
      return SettingsFormResult::Reset;
      // AI: end
    }

    if (request->hasArg(F("PIN"))) {
      const String pinValue = request->arg(F("PIN"));
      const char *pin = pinValue.c_str();
  ...
  if (subPage == SUBPAGE_UM)
  {
    if (!requestJSONBufferLock(JSON_LOCK_SETTINGS)) {
      return SettingsFormResult::Busy;
    }
  ...
  #endif
  return SettingsFormResult::Applied;
}
