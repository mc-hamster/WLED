...
From the WLED repository root, use an ignored local environment:
...
Use a separate output directory for each run. Stop serial monitors and other
...
- `results.json`: case results, bounded recovery attempts, last action metadata
  on failure, source hashes/revision, oracle type, health samples and descriptive
  heap summaries.
- `junit.xml`: CI-compatible failures and explicit skips.
...
- `baseline-state.json`: only the power, brightness and segment colors the runner
  can change. This is not a complete device-configuration backup.
...
unlocks HTTP settings to prove the PIN works, then verifies that missing, wrong
